...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        default:
            ...
            // Timer interval of 2 hours, with a 15 minute tolerance
            let timer = Timer.scheduledTimer(timeInterval: 7200.0, target: self, selector: #selector(pingLocation), userInfo: nil, repeats: true)
            timer.tolerance = 900.0
        }
    }
    ...
    func updateLocation() {
        if let serialNumber = getSerialNumber() {
            let http = HttpClient()
            http.POST(url: "\(Endpoints.development)/update/\(serialNumber)/location", body: Location.shared.getInfo()) { (err: Error?, data: Data?) in
                guard data != nil else {
